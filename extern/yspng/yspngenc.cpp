...
int YsGenericPngEncoder::WriteIDATChunk(unsigned int nLine,unsigned int bytePerLine,unsigned char dat[])
{
...
int YsGenericPngEncoder::Encode(int width,int height,int bitDepth,int colorType,unsigned char dat[])
{
...
int YsRawPngEncoder::EncodeToFile(const char fn[],int width,int height,int bitDepth,int colorType,unsigned char dat[])
{
	fp=fopen(fn,"wb");
	if(NULL!=fp)
	{
