...
*/
//----------------------------------------------------------------------
#ifndef h_ParFloat_included
#define h_ParFloat_included
//----------------------------------------------------------------------
...
class h_ParFloat : public h_Parameter
{
  ...
  public:
    ...
    h_ParFloat(h_String a_Name, h_String a_Label, float a_Value, int a_Flags,
...
class h_ParFloatPow : public h_ParFloat
{
  private:
    float m_Power;

  public:
    h_ParFloatPow(h_String a_Name, h_String a_Label, float a_Value, int a_Flags,
                 float a_Min=0, float a_Max=1, float a_Step=0, float a_Power=1)
    : h_ParFloat(a_Name,a_Label,a_Value,a_Flags,a_Min,a_Max,a_Step)
      {
        m_Power = a_Power;
      }
    ...
    virtual float getValue(void)
      {
        const float v = h_ParFloat::getValue();
        // TODO: check?
        if (v>0)
        {
