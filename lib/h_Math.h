...
lii:
- renamed the old methods to the new 'F' for 'Fast' suffix scheme
- the lines commented bellow:
  i.e. sources from:
    http://iquilezles.org/www/articles/sfrand/sfrand.htm
    http://bits.stephan-brumme.com/invSquareRoot.html
    etc..

  were implemented in a simmilar fashion in axonlib
- unless H_INLINE_MATH is defined, all methods in this header are not inlined.
...
// TODO/idea:
// split & make obvious, the differences between:
// for example:
...
#ifdef H_INLINE_MATH
  #define __h_math_inline
#else
  #define __h_math_inline inline
#endif
