...
#define _CMATH_ALIGN(x)   __attribute__ ((aligned(x)))
  ...
  #define _CNUM_ALIAS __attribute__((__may_alias__))
#else
  #define _CNUM_ALIAS
#endif
...
{
  return cnum_new(x.r * h_Cos(x.i), x.r * h_Sin(x.i));
}

_CNUM_INLINE
cnum_s cnum_polar(const cnum_s x)
{
  return cnum_new(h_Cabs(x.r, x.i), h_Carg(x.r, x.i));
}
...
#define _CNUM_CHECK_EXP_D_ \
  h_Abs(deg - h_Round(deg)) == 0

_CNUM_INLINE
cnum_s cnum_exp(const cnum_s x)
{
  cnum_t sin_i = h_Sin(x.i);
  cnum_t cos_i = h_Cos(x.i);
  const cnum_t exp_r = h_Exp(x.r);
...
_CNUM_INLINE
cnum_s cnum_pow_c_k(const cnum_s x, const cnum_s y, const int k)
{
  _CNUM_CHECK_POW_C_;
...
{
  const cnum_t r_pow_n = h_Pow(h_Cabs(x.r, x.i), n);
  const cnum_t theta_n = h_Carg(x.r, x.i) * n;
  if (n == 0)
    return cnum_new(1, 0);
  if (n == 1)
    return x;
  return cnum_new(r_pow_n * h_Cos(theta_n), r_pow_n * h_Sin(theta_n));
}
...
#define cnum_cosh(x) \
  cnum_new(h_Cosh(x.r)*h_Cos(x.i), h_Sinh(x.r)*h_Sin(x.i))
